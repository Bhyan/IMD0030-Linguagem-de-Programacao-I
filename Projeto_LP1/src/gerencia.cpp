...
namespace PetFera{
    ...
    */
    ...
    /**
    * @brief    Método que escreve dados de animais em um aquivo .csv 
    */
    void Gerencia::escrever_animais()
    {
        ofstream func("data/animais.csv", ios::trunc);

        if(func.is_open())
        {
            for(auto i = m_lista_animal.begin(); i != m_lista_animal.end(); i++)
            {
                func << (i->second)->getId() << ";" << (i->second)->getClasse() << ";"
                    << (i->second)->getNome() << ";" << (i->second)->getCientifico() << ";"
                    << (i->second)->getSexo() << ";" << (i->second)->getTamanho() << ";"
                    << (i->second)->getDieta() << ";" << (i->second)->getVeterinario()->getId() << ";"
                    << (i->second)->getTratador()->getId() << ";" << (i->second)->getBatismo()<< ";";


                if((i->second)->getClasse().compare("Amphibia") == 0)
                {
                    shared_ptr<Anfibio> a = static_pointer_cast<Anfibio>(i->second);
                    func << a->getTotalMudas() << ";" << a->getUltimaMuda() << ";";
                    
                    shared_ptr<AnfibioNativo> aNativo = static_pointer_cast<AnfibioNativo>(a);

                    func << aNativo->getIbama() << ";";

                    if(aNativo->getAutorizacao().compare("-") == 0)
                    {
                        shared_ptr<AnfibioExotico> aExotico = static_pointer_cast<AnfibioExotico>(a);
                        func << aExotico->getPais() << ";-;" << endl;
                    }
                    else
                    {
                        func << aNativo->getOrigem() << ";" << aNativo->getAutorizacao() << ";" << endl;
                    }

                }
                // else if((i->second)->getClasse().compare("Aves") == 0)
                // {
                //     shared_ptr<Ave> ave = static_pointer_cast<Ave>(i->second);
                //     func << ave->getTamanhoBico() << ";" << ave->getEnvergadura() << ";";

                //     shared_ptr<AveNativa> aNativo = static_pointer_cast<AveNativa>(ave);

                //     func << aNativo->getIbama() << ";";

                //     if(aNativo->getAutorizacao().compare("-") == 0)
                //     {
                //         shared_ptr<AveExotica> aExotica = static_pointer_cast<AveExotica>(ave);
                //         func << aExotica->getPais() << ";-;" << endl;
                //     }
                //     else
                //     {
                //         func << aNativo->getOrigem() << ";" << aNativo->getAutorizacao() << ";" << endl;
                //     }
                // }
                // else if((i->second)->getClasse().compare("Mammalia") == 0)
                // {
                //     shared_ptr<Mamifero> mam = static_pointer_cast<Mamifero>(i->second);
                //     func << mam->getCor() << ";";

                //     shared_ptr<MamiferoNativo> aNativo = static_pointer_cast<MamiferoNativo>(mam);

                //     func << aNativo->getIbama() << ";";

                //     if(aNativo->getAutorizacao().compare("-") == 0)
                //     {
                //         shared_ptr<MamiferoExotico> aExotico = static_pointer_cast<MamiferoExotico>(mam);
                //         func << aExotico->getPais() << ";-;" << endl;
                //     }
                //     else
                //     {
                ...
                //     }
                // }
                // else if((i->second)->getClasse().compare("Reptilia") == 0)
                // {
                //     shared_ptr<Reptil> rep = static_pointer_cast<Reptil>(i->second);
                //     if(rep->getVeneno())
                //     {
                //         func << "Sim;";
                //     }
                ...
                //     func << rep->getTipoVeneno() << ";";

                //     shared_ptr<ReptilNativo> aNativo = static_pointer_cast<ReptilNativo>(rep);

                //     func << aNativo->getIbama() << ";";

                //     if(aNativo->getAutorizacao().compare("-") == 0)
                //     {
                //         shared_ptr<ReptilExotico> aExotico = static_pointer_cast<ReptilExotico>(rep);
                //         func << aExotico->getPais() << ";-;" << endl;     
                //     }
                //     else
                //     {
    ...
    */
            ...
            try{
                ...
                switch(op){
                    ...
                    case 6:
                        ...
                        cin >> op_aux;
                        cin.ignore(1, '\n'); 
                        editar_animal(buscar_animal(op_aux));
                        break;
                    case 7:
                        cout << "Digite o ID do animal: ";
                        cin >> op_aux;
    ...
    /**
     * @brief   Método editar animal.
     * @details Os parâmetros gerais editados são tamanho, dieta, veterinário, tratador é o total de
     * mudas e data da ultima muda dos anfibios e tamanho do bico e envergadura para aves.
     * @params Map com o animal.
     */
    void Gerencia::editar_animal(map<int, shared_ptr<Animal>> animal_){
        string aux, dieta_;
        int veterinario_, tratador_;
        float tamanho_;
        bool flag;
        auto i = animal_.begin();

        // Editar tamanho.
        cout << "Tamanho (Para não alterar valor digite 0): ";
        getline(cin, aux);
        try{
            tamanho_ = stoi(aux);
        }
        catch(invalid_argument &erro){
            cout << "Entrada invalida. Informe um valor inteiro.";
        }
        if(tamanho_ != 0){
            (i -> second) -> setTamanho(tamanho_);
        }// Fim editar tamanho.

        // Editar dieta.
        cout << "Dieta (Se não for alterado digite ENTER): ";
        getline(cin, dieta_);
        if(dieta_.size() != 0){
            (i -> second) -> setTamanho(tamanho_);
        }// Fim editar dieta.
        ...
        // Editar veterinário.
        cout << "Veterinário (Para não alterar valor digite 0): ";
        getline(cin, aux);
        try{
            veterinario_ = stoi(aux);
        }
        catch(invalid_argument &erro){
            cout << "Entrada invalida. Informe um valor inteiro.";
        }
        if(veterinario_ != 0){
            auto funcionario = buscar_funcionario(veterinario_).begin();
            (i -> second) -> setVeterinario(funcionario -> second);
        }// Fim veterinário.

        // Editar tratador.
        cout << "Tratador (Para não alterar valor digite 0): ";
        getline(cin, aux);
        try{
            tratador_ = stoi(aux);
        }
        ...
        if(tratador_ != 0){
            auto funcionario = buscar_funcionario(tratador_).begin();
            (i -> second) -> setTratador(funcionario -> second);
        }// Fim tratador.
        ...
        if((i->second) -> getClasse().compare("Amphibia") == 0){
            int total_muda_;
            string ultima_muda_;
            shared_ptr<Anfibio> animal = static_pointer_cast<Anfibio> (i -> second);

            do{
                cout << "Informe o nova quantidade de mudas: ";
                getline(cin, aux);
                try{
                    total_muda_ = stoi(aux);
                    flag = true;
                }
                catch(invalid_argument &erro){
                cout << "Entrada invalida. Informe um valor inteiro." << endl;
            ...
            do{
                cout << "Informe a data da ultima muda (dd/mm/aaaa): ";
                getline(cin, ultima_muda_);
                try{
                    if(ultima_muda_.size() == 0) throw ErroCadastro();
                    flag = true;
                }
                catch(ErroCadastro &erro){
                cout << erro.what();
            ...
            animal -> setUltimaMuda(ultima_muda_);
            animal -> setTotalMuda(total_muda_);
        }// Fim anfíbio.

        // Editar ave.
        else if((i->second) -> getClasse().compare("Aves") == 0){
            int tamanho_bico_, envergadura_;
            shared_ptr<Ave> animal = static_pointer_cast<Ave> (i -> second);

            do{
                cout << "Informe o tamanho do bico: ";
                getline(cin, aux);
                try{
                    tamanho_bico_ = stoi(aux);
                    flag = true;
                }
                catch(invalid_argument &erro){
                cout << "Entrada invalida. Informe um valor inteiro." << endl;
            ...
            do{
                cout << "Informe a envergadura: ";
                getline(cin, aux);
                try{
                    envergadura_ = stoi(aux);
                    flag = true;
                }
                catch(invalid_argument &erro){
                cout << "Entrada invalida. Informe um valor inteiro." << endl;
            ...
            animal -> setTamanhoBico(tamanho_bico_);
            animal -> setEnvergadura(envergadura_);
        }//Fim ave.
