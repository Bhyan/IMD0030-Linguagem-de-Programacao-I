/**
 * @file lista.h
 * @brief Protótipo da classe ListaLigada.
...
public:
	...
	friend std::ostream& operator<< <T>( std::ostream&, ListaLigada<T> const &l);
   	...
   	shared_ptr<Node<T>> getCabeca(); // Adicionado o guardião cabeça.
    shared_ptr<Node<T>> getCauda(); // Adicionado o guardião calda.
};
